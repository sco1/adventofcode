...
% Check validity for Part 1, where the keys represent the (inclusive) range of the number of key
% letters in the password
n_letter = count(spec.password, spec.key_letter);
is_valid_pt1 = (spec.key_l <= n_letter) & ...
    (n_letter <= spec.key_r);

% Check validity for Part 2, where the keys represent indices to check against the key letter;
% exactly one of these indices must contain the key, so we have an XOR
is_valid_pt2 = xor( ...
